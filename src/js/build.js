var build = function(path, callback) {
    ...
    for (i = 0; i < game.modules.length; i++) {
        ...
        totalSize += size;
        console.log(file + ' ' + (size.toString()) + ' bytes');
    }
    console.log('Total ' + (totalSize.toString()) + ' bytes');
    ...
    fs.writeFile(outputFile, output, function(err) {
        if (err) {
            ...
            // callback(false, 'Saved ' + outputFile + ' ' + (size.toString()) + ' bytes (' + percent + '%)');
            callback(false, 'Build completed');
        }
    });
};
